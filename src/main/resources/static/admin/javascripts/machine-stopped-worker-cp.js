$(function () {
    ...
    $("#selectLineSearch").change($("#selectLineSearch").val(), function () {
        machineStopped.getAllMachineNameByLineName($("#selectLineSearch").val(), function (response) {
            $('#selectMachineSearch').empty();
            $("#selectMachineSearch").append("<option value='.*[A-Za-z0-9-]*.'>설비</option>");
            if(response.CODE == "7777"){
                $.each(response.DATA, function(key, value){
    ...
    var btnFinish = $('<button></button>').text('끝').addClass('btn btn-info').on('click', function () {
        ...
        machineStopped.updateAlarmList(rowKeyIn, function(response){
            machineStopped.getAllAlarmByStatus();
        });
    });
    ...
    machineStopped.getAlarmList = function (rowKey, callback) {
        $.ajax({
            url: "/v1/api/fukoku/machine-stopped/get-one-as-list",
            type: 'GET',
            dataType: 'JSON',
            data: {
                "rowKey"    :   rowKey
    ...
    machineStopped.addErrorState = function (data, callback) {
        $.ajax({
            url: "/v1/api/fukoku/error-state/",
            type: 'POST',
            dataType: 'JSON',
            data : JSON.stringify(data),
            beforeSend: function (xhr) {
    ...
    $("#btnNotError").click(function () {
        ...
        if(json=='N'){
            ...
            var rowKeyIn = {"ROW_KEY" : json.toString()};
            machineStopped.updateAlarmList(rowKeyIn, function(response){
                machineStopped.getAllAlarmByStatus();
            });
            machineStopped.getAlarmList(json.toString(), function(response){
                $.each(response.DATA, function (key, value) {
                    var d = {
                    ...
                    machineStopped.addNonMovingState(d, function(response){
                        console.log('Ok');
                    });
                });
            });
    ...
    $("#btnTransferList").click(function () {
        var json = deletedWithCheckBox().length==0?"N":deletedWithCheckBox();
        if(json=='N'){
            alert('Please check the record!')
            return;
            ...
            var rowKeyIn = {"ROW_KEY" : json.toString()};
            machineStopped.updateAlarmDepartmentList(rowKeyIn, function(response){
                machineStopped.getAllAlarmByStatus();
            });
        }
    });
    ...
    $("#btnErrorInputList").click(function () {
        ...
        if(j=='N'){
            ...
            machineStopped.getAlarmList(j.toString(), function(response){
                record = response;
                $.each(response.DATA, function (key, value) {
                    $("#R").append("<tr><td>" + value.LINE_NAME + "</td><td>" + value.MACHINE_NAME + "</td><td>" + value.MACHINE_STATE + "</td><td>" + value.START_TIME + "</td><td>" + value.END_TIME + "</td><td>" + value.DURATION + "</td><td>" + value.ALARM_CODE + "</td><td>" + value.ALARM_NAME + "</td></tr>");
                });
            });
            $("#smartwizard").smartWizard('reset');
            $("#modalMachineStopped").modal('show');
    ...
    function addErorStateModule(dErrorState){
        ...
        $.each(dErrorState.DATA, function (key, value) {
            ...
            machineStopped.addErrorState(data, function () {
                changeText();
                $("#modalMachineStopped").modal('hide');
                $('#smartwizard').smartWizard("reset");
            });
