$(function() {
    ...
    dashboard.getGaugesData = function(selector,line,machineName,startTime,endTime){
        ...
        $.ajax({
            url:"/v1/api/fukoku/dashboard/find-stop-time",
            type:'GET',
            dataType:'JSON',
            data:{
                "lineName": line,
            ...
            success: function(response){
                ...
                if(response.CODE == "7777"){
                    if(response.DATA != null){
                        ...
                        dashboard.setUpdateGauges(gauges1,response.DATA[0].FACILITY_EFFICIENCY_RATE.toFixed(2));
                        dashboard.setUpdateGauges(gauges2,response.DATA[0].TIME_OPERATING_RATE.toFixed(2));
                        dashboard.setUpdateGauges(gauges3,response.DATA[0].PERFORMANCE_START_UP_RATE.toFixed(2));
                        dashboard.setUpdateGauges(gauges4,response.DATA[0].QUALITY_ITEM_RATE.toFixed(2));

                        parentNode.find(".txtTotalFacilly").val(response.DATA[0].FACILITY_EFFICIENCY_RATE.toFixed(2) + " %");
                        parentNode.find(".txtTotalOperatingTime").val(response.DATA[0].TIME_OPERATING_RATE.toFixed(2) + " %");
                        parentNode.find(".txtTotalPerformanceStartUp").val(response.DATA[0].PERFORMANCE_START_UP_RATE.toFixed(2) + " %");
                        parentNode.find(".txtTotalQuality").val(response.DATA[0].QUALITY_ITEM_RATE.toFixed(2) + " %");
                        parentNode.find(".txtOperatingTime").val(response.DATA[0].OPERATING_TIME + " mins");
                        parentNode.find(".txtPlanStopTime").val(response.DATA[0].PLAN_STOP_TIME + " mins");
                        parentNode.find(".txtUnplanStopTime").val(response.DATA[0].UNPLAN_STOP_TIME.toFixed(2) + " mins");
                        parentNode.find(".txtCycleTime").val(response.DATA[0].CYCLE_TIME.toFixed(2) + " mins");
                        parentNode.find(".txtProductQuantity").val(response.DATA[0].PRODUCT_QUANTITY + " unit");
                        parentNode.find(".txtDefectionProduct").val(response.DATA[0].DEFECTIVE_PRODUCT + " unit");
                        closeLoading();
                    }
